...
- **Follow role boundaries** defined in `.ai/agents/`
...
All implementations must follow standards from `.ai/standards/`:
...
- Leverage Gemini's multimodal capabilities when appropriate
- Use structured outputs (JSON mode) for consistent responses
- Be mindful of context window limits
- Utilize function calling for complex operations

### Best Practices

- Use appropriate model variants (Gemini Pro, Flash) based on task complexity
- Implement proper token counting for cost estimation
